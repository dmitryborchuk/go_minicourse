...
func (c *Command) Do() error {
	...
	case "change_name":
		return c.changeAccountsName()
	case "change_balance":
		return c.changeAccountsBalance()
	default:
		return fmt.Errorf("unknown command: %s", c.Cmd)
	}
}

func (c *Command) createAccount() error {
...
func (c *Command) getAccount() error {
	resp, err := http.Get(
...
func (c *Command) changeAccountsName() error {
	request := dto.ChangeAccountsNameRequest{
		Name:    c.Name,
		NewName: c.NewName,
	}
	...
	resp, err := http.Post(
		fmt.Sprintf("http://%s:%d/account/change/name", c.Host, c.Port),
		"application/json",
		bytes.NewReader(data),
	)
	if err != nil {
...
func (c *Command) changeAccountsBalance() error {
	request := dto.ChangeAccountsBalanceRequest{
		Name:   c.Name,
		Amount: c.Amount,
	}
	...
	resp, err := http.Post(
		fmt.Sprintf("http://%s:%d/account/change/balance", c.Host, c.Port),
		"application/json",
		bytes.NewReader(data),
	)
	if err != nil {
